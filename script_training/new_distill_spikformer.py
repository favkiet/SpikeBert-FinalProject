...
def get_model_and_tokenizer(model_name, model_type, num_labels):
    ...
    if model_type == "bert":
        ...
        model = BertForSequenceClassification.from_pretrained(model_name, num_labels=num_labels, output_hidden_states=True)
...
def distill(args):
    ...
    # Lấy teacher model và tokenizer
    tokenizer, teacher_model = get_model_and_tokenizer(args.teacher_model_path, args.model_type, args.label_num)
    ...
    if args.use_csv:
        if args.data_augment == "True":
            print("With Augmentation (CSV)")
            train_dataset = CSVDataset(data_path=f"/kaggle/input/{args.dataset_name}/train_augment.csv",
                                      sentence_col=args.sentence_col, 
                                      label_col=args.label_col)
        else:
            print("Without Augmentation (CSV)")
        ...
        if args.data_augment == "True":
            ...
            train_dataset = TxtDataset(data_path=f"/kaggle/input/{args.dataset_name}/train_augment.txt")
        else:
    ...
    print(f"Using teacher model: {args.teacher_model_path} (type: {args.model_type})")
    if len(device_ids) > 1:
    ...
    for epoch in tqdm(range(args.epochs)):
        ...
        for batch in tqdm(train_data_loader):
            ...
            # Lấy embeddings từ teacher model
            if args.model_type == "bert":
                tea_embeddings = teacher_model.bert.embeddings.word_embeddings.weight
            else:  # modernbert
                # ModernBERT có thể có cấu trúc khác, cần điều chỉnh tùy theo model
                tea_embeddings = teacher_model.model.embeddings.word_embeddings.weight
            ...
            # Lấy hidden states từ teacher model
            ...
            # Sử dụng temperature cho knowledge distillation
            logit_loss = F.kl_div(
            ...
            # Kết hợp loss với alpha parameter
        ...
        if args.metric == "acc":
            ...
            print(f"Epoch {epoch}, Test MCC: {mcc:.4f}")
        ...
        if record >= np.max(metric_list):
            torch.save(student_model.state_dict(),
                    f"saved_models/distilled_spikformer/{args.model_type}_{args.dataset_name}_epoch{epoch}_{record}" + 
