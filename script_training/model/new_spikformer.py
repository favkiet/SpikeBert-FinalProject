...
class mlp(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.transpose(0, 1) # T B L D
        T, B, L, D = x.shape
        x = x.flatten(0, 1) # TBL * D

        # ---- Sửa lỗi ở đây ----
        x = self.fc1(x) # TBL * hidden_features
        x = self.ln1(x) # TBL * hidden_features
        x = x.reshape(T, B, L, self.hidden_features).contiguous() # T * B * L * hidden_features
        x = self.lif1(x)

        x = x.flatten(0, 1) # TBL * hidden_features

        # Sửa lỗi tương tự cho lớp thứ hai
        x = self.fc2(x) # TBL * out_features
        x = self.ln2(x) # TBL * out_features
        x = x.reshape(T, B, L, self.out_features).contiguous() # T * B * L * out_features (ở đây out_features = D)
        x = self.lif2(x)
