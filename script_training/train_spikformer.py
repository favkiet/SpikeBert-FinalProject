...
def get_tokenizer(tokenizer_path, model_type):
    """Lấy tokenizer dựa trên loại model"""
    if model_type == "bert":
...
def train(args):
    ...
    tokenizer = get_tokenizer(args.tokenizer_path, args.model_type)
    ...
    valid_data_loader = DataLoader(
        dataset=valid_dataset, batch_size=args.batch_size, shuffle=False, drop_last=False)

    device_ids = [i for i in range(torch.cuda.device_count())] if device.type == "cuda" else []
    print(f"Available devices: {device_ids}")
    print(f"Using tokenizer: {args.tokenizer_path} (type: {args.model_type})")
    if len(device_ids) > 1:
    ...
    for epoch in tqdm(range(args.epochs)):
        ...
        if acc >= np.max(acc_list):
            torch.save(model.state_dict(),
                       f"saved_models/trained_spikformer/{args.model_type}_{args.dataset_name}_epoch{epoch}_{acc}" +
