...
def convert_label(label):
    """Chuyển đổi label từ int sang sentiment cho SST-2"""
    if label == 0:
        return "negative"
    elif label == 1:
        return "positive"
...
def save_sst2_to_files(data_dict, output_dir):
    """Lưu dữ liệu SST-2 vào các file txt và CSV theo format SpikeBERT"""
    ...
    df.to_csv(os.path.join(output_dir, 'sst2_dataset.csv'), index=False, encoding='utf-8')
    
    print(f"Đã lưu dữ liệu SST-2 vào: {output_dir}")
    ...
    print(f"- {output_dir}/sst2_dataset.csv")

def main():
    """Hàm chính để chuẩn bị dữ liệu SST-2"""
    print("Bắt đầu chuẩn bị dữ liệu SST-2...")
    
    # Tạo thư mục data/sst2 nếu chưa tồn tại
    os.makedirs("data/sst2", exist_ok=True)
    
    try:
        # Load dataset SST-2
        print("Đang tải dataset SST-2...")
        ...
        # Lưu dữ liệu
        save_sst2_to_files(all_data, "data/sst2")
        
        print("Hoàn thành chuẩn bị dữ liệu SST-2!")
